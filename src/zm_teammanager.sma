...
public zm_onInit() {
    zm_registerExtension("[ZM] Team Manager",
                         PLUGIN_VERSION,
                         "Controls who is a zombie and who isn't");
...
initializeForwards() {
    ...
    g_fw[onRefresh]           = CreateMultiForward("zm_onRefresh",
                                                   ET_IGNORE,
                                                   FP_CELL,
                                                   FP_CELL);
    g_fw[onPlayerSpawn]       = CreateMultiForward("zm_onPlayerSpawn",
                                                   ET_IGNORE,
                                                   FP_CELL, 
                                                   FP_CELL);
    g_fw[onPlayerDeath]       = CreateMultiForward("zm_onPlayerDeath",
                                                   ET_IGNORE,
                                                   FP_CELL,
                                                   FP_CELL);
    g_fw[onTeamChangeBlocked] = CreateMultiForward("zm_onTeamChangeBlocked",
                                                   ET_IGNORE,
                                                   FP_CELL);
...
initializeInfectForwards() {
    g_fw[onBeforeInfected] = CreateMultiForward("zm_onBeforeInfected",
                                                ET_CONTINUE,
                                                FP_CELL,
                                                FP_CELL,
                                                FP_CELL);
    g_fw[onInfected]       = CreateMultiForward("zm_onInfected",
                                                ET_IGNORE,
                                                FP_CELL,
                                                FP_CELL);
    g_fw[onAfterInfected]  = CreateMultiForward("zm_onAfterInfected",
                                                ET_IGNORE,
                                                FP_CELL,
                                                FP_CELL);
}
...
initializeCureForwards() {
    g_fw[onBeforeCured] = CreateMultiForward("zm_onBeforeCured",
                                             ET_CONTINUE,
                                             FP_CELL,
                                             FP_CELL,
                                             FP_CELL);
    g_fw[onCured]       = CreateMultiForward("zm_onCured",
                                             ET_IGNORE,
                                             FP_CELL,
                                             FP_CELL);
    g_fw[onAfterCured]  = CreateMultiForward("zm_onAfterCured",
                                             ET_IGNORE,
                                             FP_CELL,
                                             FP_CELL);
}
...
public msgTeamInfo(const msgID, const msgDest) {
    ...
    get_user_name(id, szIdName, 31);
#endif
    if (!getFlag(g_playerFlag[flag_FirstTeamSet],id) && (team[0] == 'T'
            || team[0] == 'C')) {
