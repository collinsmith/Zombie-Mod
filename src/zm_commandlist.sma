...
public zm_onInit() {
    zm_registerExtension("[ZM] Command List",
                         PLUGIN_VERSION,
...
public zm_onRegisterCommands() {
    new ZM_CMD:commands = zm_registerCommand("commands",
                                            "displayCommandList",
                                            "abcdef",
                                            "Displays a printed list of all commands");
    zm_registerCommandAlias(commands, "cmds");
    
    new ZM_CMD:commandList = zm_registerCommand("commandlist",
                                                "displayCommandMotD",
                                                "abcdef",
...
public zm_onCommandRegistered(ZM_CMD:cmdId, const command[], const handle[], const flags[], const description[], const adminFlags) {
    new tempstring[128];
    formatex(tempstring, 127, "<tr><td>%s</td><td>: %s</td></tr>",
                              command,
                              description);
